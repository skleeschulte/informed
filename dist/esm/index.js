import React, { useState, useContext, useMemo, useRef, useEffect, useLayoutEffect, Component } from 'react';
...
function () {
  ...
  _createClass(ObjectMap, null, [{
    ...
    value: function pullOut(object, path, index) {
      ...
      debug$1('PathArray', pathArray); // Get the array
...
var useForm = function useForm(_ref) {
  ...
  var _useState3 = useState(formController.getFormState()),
      ...
      setFormState = _useState4[1]; // Initialize form stuff ( hack for constructor code muhahaha )


  useState(function () {
    // Update the form state to trigger rerender!
    formController.on('change', function () {
...
var Form = function Form(_ref) {
  ...
  var getWrappedContent = function getWrappedContent() {
...
var buildFieldApi = function buildFieldApi(formApi, field, updater) {
  var fieldApi = updater.getField(field).fieldApi;
  return {
...
function useFieldApi(field) {
  var formApi = useFormApi();
  var updater = useContext(FormRegisterContext);
  var fieldApi = useMemo(function () {
    return buildFieldApi(formApi, field, updater);
  }, [field]);
  return fieldApi;
}
...
function useField(field) {
  ...
  var validate = fieldProps.validate,
      ...
      type = fieldProps.type; // Grab the form register context
...
var asField = function asField(Component$$1) {
  return function (props) {
    var field = props.field,
        ...
        allowEmptyString = props.allowEmptyString,
        ...
        format = props.format,
        parse = props.parse,
        rest = _objectWithoutProperties(props, ["field", "validate", "initialValue", "validateOnChange", "validateOnBlur", "validateOnMount", "maskOnBlur", "allowEmptyString", "onValueChange", "notify", "keepState", "maintainCursor", "debug", "type", "mask", "format", "parse"]);
